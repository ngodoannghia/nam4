...
class env():
    ...
    def get_b(self):
        ...
        b = self.state[1]
        #print("get_b:: ","t = ", self.time,"g = ",self.g, "e = ", self.state[3], "b = ", b, " d_op:", self.d_op, " d = ", self.d)
        if self.d_op > b:
            return b + self.g
        else:
            if self.g >= self.d:
    ...
    def get_e(self):
        ...
        return 1
    def init_state(self):
        return [self.get_lambda(), np.random.uniform(self.b_low, self.b_high), self.get_h(), self.get_e()]
    def get_time(self):
        self.time += 0.25
        if self.time == 24:
            self.time = 0
    ...
    def cal(self, action):
        ...
        if b <= d_op + 150:
         ...
         #   print("d_com: ", self.get_dcom(self.max_number_of_server, lamda))
    ...
    def reward_func(self, action):
        ...
        #print("reward_func: ","d_op: ", self.d_op, "b: ", b)
        if self.d_op > b:  
            cost_batery = 0
    ...
    def step(self, action):
       ...
       # print("m =", self.m, "mu =", self.mu, "d_com: ", self.d_com)
